...
for (const p of people) {
  ...
  for (const [cs, ce] of chunks(START, END)) {
    ...
    while (url) {
      let page
      try { page = await graph(url) } catch (e) { if (e.status === 403) { blocked403.add(p.guid); url = ''; break } if (e.status === 404) { url = ''; break } throw e }
