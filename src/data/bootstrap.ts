...
function hydrate({ aRows, pRows, sRows, cRows, asRows }: Rows): BootResult['counts'] {
  ...
  // The board keeps full history; only current placements count as consultants
  // (Adam, 26 Aug + TN source check 28 Aug): Alumni, Ex Associates and
  // offboarded people are history, not headcount.
  asRows.forEach((as) => { if (as.account_id && !/alumni|ex associate|offboarded/i.test(as.placement_status || '')) consPerAccount.set(as.account_id, (consPerAccount.get(as.account_id) || 0) + 1) })
